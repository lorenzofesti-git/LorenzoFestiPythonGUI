...
from Int_solver_panels import COMPUTE_KL_VPM
from streamlines import STREAMLINE_VPM
from Circulation import COMPUTE_CIRCULATION


def pannelli(Vinf, AoA, XF, YF, xu, yu, xl, yl):
    # Convert angle of attack to radians
    AoAR = AoA * (np.pi / 180)
    x0 = 0.5
    y0 = 0
    items = 20
    XB1 = np.zeros((items, 1), dtype=float64)
    YB1 = np.zeros((items, 1), dtype=float64)
    XB2 = np.zeros((items, 1), dtype=float64)
    YB2 = np.zeros((items, 1), dtype=float64)
    check1 = np.zeros((items, 1), dtype=float64)
    check2 = np.zeros((items, 1), dtype=float64)
    k = 0
    eps = 0.01
    xcircle = np.zeros((items, 1), dtype=float64)
    while k < items:
        xcircle[k] = x0 - x0 * cos(np.pi * k / items)
        k += 1

    for i in range(len(xu)):
        for j in range(len(xcircle)):
            if (xcircle[j] > xu[i] - eps) & (xcircle[j] < xu[i] + eps):
                XB1[j] = xcircle[j]
                YB1[j] = yu[i]
            if sqrt((XB1[j] ** 2) + (YB1[j] ** 2)) > 10 * eps:
                check1[j] = 1

    for i in range(len(xl)):
        for j in range(len(xcircle)):
            if (xcircle[j] > xl[i] - eps) & (xcircle[j] < xl[i] + eps):
                XB2[j] = xcircle[j]
                YB2[j] = yl[i]
            if sqrt((XB2[j] ** 2) + (YB2[j] ** 2)) > 10 * eps:
                check2[j] = 1

    XB = np.concatenate((np.flip(XB1), XB2))
    YB = np.concatenate((np.flip(YB1), YB2))
    np.savetxt("some_i1.txt", (XB))
    np.savetxt("some_i2.txt", (YB))
    ...
    for i in range(numPan):  # Loop over all i panels
        ...
        for j in range(numPan):  # Loop over all j panels
            addVal = (
                addVal - (gamma[j] / (2 * np.pi)) * L[i, j]
            )  # Sum all tangential vortex panel terms

        Vt[i] = (
            Vinf * np.sin(beta[i]) + addVal + gamma[i] / 2
    ...
    # Grid parameters
    nGridX = 50  # X-grid for streamlines and contours
    nGridY = 50  # Y-grid for streamlines and contours
    ...
    Ysl = np.linspace(
        yVals[0], yVals[1], int((slPct / 100) * nGridY)
    ...
    # AF_S=np.array2string(AF)                                                   # Concatenate XB and YB geometry points
    ...
    for m in range(nGridX):  # Loop over X-grid points
        for n in range(nGridY):  # Loop over Y-grid points
            ...
            if afPath.contains_points([(XP, YP)]):  # If (XP,YP) is in the body
                ...
                Vx[m, n] = Vinf * np.cos(AoAR) + sum(
                    -gamma * Nx / (2 * np.pi)
                )  # Compute X-velocity
                Vy[m, n] = Vinf * np.sin(AoAR) + sum(
                    -gamma * Ny / (2 * np.pi)
                )  # Compute Y-velocity
