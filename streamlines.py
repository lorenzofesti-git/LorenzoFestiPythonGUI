...
def STREAMLINE_VPM(XP, YP, XB, YB, phi, S):
    ...
    for j in range(numPan):  # Loop over all panels
        ...
        E = math.sqrt(max(B - A**2, 0))  # E term
        if (
            ...
            # Compute Nx, Ref
            term1 = (
                0.5 * Cx * np.log(max((S[j] ** 2 + 2 * A * S[j] + B) / B, eps))
            ...
            # Compute Ny, Ref
            term1 = (
                0.5 * Cy * np.log((S[j] ** 2 + 2 * A * S[j] + B) / B)
        ...
        if (
            np.iscomplex(Nx[j]) or np.isnan(Nx[j]) or np.isinf(Nx[j])
        ):  # If Nx term is complex or a NAN or an INF
            Nx[j] = 0  # Set Nx value equal to zero
        if (
            np.iscomplex(Ny[j]) or np.isnan(Ny[j]) or np.isinf(Ny[j])
